...
The homepage includes combined category and publisher filters so visitors can narrow the game catalog without leaving the listing page; the controls are keyboard-friendly and keep the page content accessible.
...
Repository-wide coding standards are documented in the [Copilot instructions](.github/copilot-instructions.md) and the technology-specific files in [`.github/instructions`](.github/instructions). In particular:
...
- Reusable Astro components document their `Props` interface and public inputs.
- TypeScript uses a consistent quote style within each file, semicolons, multiline trailing commas, and two-space indentation; ESLint enforces the automatically checkable formatting rules.
